...
async def test_orchestrator_has_no_evidence_without_database_and_removes_fake_citations():
    result = await AgentOrchestrator(Settings(_env_file=None), BadCitationProvider()).answer("What is a fictional policy?", development_context(AgentScope.COMMITTEE))
...
async def test_stream_emits_safe_sse_contract_events():
    events = [event async for event in AgentOrchestrator(Settings(_env_file=None), BadCitationProvider()).stream("What is a fictional policy?", development_context(AgentScope.PUBLIC))]
...
async def test_empty_evidence_does_not_invoke_model():
    ...
    result = await AgentOrchestrator(Settings(_env_file=None), provider).answer("What is a fictional policy?", development_context(AgentScope.PUBLIC))
...
async def test_empty_evidence_does_not_invoke_openai_client():
    ...
    provider = OpenAIProvider(Settings(_env_file=None, ai_api_key="test-key"), type("Client", (), {"responses": responses})())
    result = await AgentOrchestrator(Settings(_env_file=None), provider).answer("What is a fictional policy?", development_context(AgentScope.PUBLIC))
...
async def test_stream_terminates_safely_on_retrieval_failure():
    events = [event async for event in AgentOrchestrator(Settings(_env_file=None), BadCitationProvider(), BrokenDataClient()).stream("What is EFDS?", development_context(AgentScope.PUBLIC))]
