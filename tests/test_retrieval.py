...
def test_source_article_relevance_prefers_title_and_phrase_matches():
    relevant = source_article_relevance(
        "Speaker Events Process", "External speaker process", ["inviting", "external", "speaker"]
    )
    unrelated = source_article_relevance(
        "Alcohol Guidelines", "Guidance for external events", ["inviting", "external", "speaker"]
    )
...
async def test_knowledge_evidence_keeps_source_provenance():
    client = FakeClient(
        {
            "knowledge_requirements": [
                {
                    "id": "req-1",
                    "requirement_text": "Submit the speaker approval form.",
                    "requirement_type": "approval",
                    "source_article_id": "article-1",
                    "source_url": "https://icu.example/article",
                    "source_content_hash": "abc",
                    "source_updated_at": "2026-08-01T00:00:00Z",
                    "evidence_text": "approval form",
    ...
    result = await KnowledgeAdapter(client).search(
        RetrievalRequest(question="What is the requirement?", terms=["speaker"], max_results=4),
...
async def test_resource_question_does_not_search_unrelated_process_tables():
    client = FakeClient(
        {
            "knowledge_resources": [
                {
                    "id": "resource-1",
                    "name": "Committee admin request form",
                    "resource_type": "form",
                    ...
                    "is_stale": False,
                    "visibility": "committee",
                    "description": "Admin request form",
                }
            ]
        }
    )
    result = await KnowledgeAdapter(client).search(
        RetrievalRequest(
            question="Where can I find the committee admin request form?",
            terms=["committee", "admin", "request", "form"],
        ),
        development_context(AgentScope.COMMITTEE),
    )
...
async def test_member_cannot_query_slack_even_with_fake_rows():
    ...
    result = await SlackAdapter(client).search(
        RetrievalRequest(question="Slack", terms=["private"]), development_context(AgentScope.MEMBER)
    )
    assert result.evidence == []
...
async def test_admin_slack_evidence_assembles_thread():
            ...
            "slack_messages": [
                {
                    ...
                    "slack_ts": "2",
                    "thread_ts": "1",
                    "message_text": "speaker reply",
                    "source_posted_at": "2026-08-01T00:01:00Z",
    ...
    result = await SlackAdapter(client).search(
        RetrievalRequest(question="discussion", terms=["speaker"], max_results=4), development_context(AgentScope.ADMIN)
    )
...
async def test_document_evidence_redacts_absolute_windows_paths():
            ...
            "documents": [
                {
                    ...
                    "raw_text": "See C:\\Users\\slee7\\private\\plan.docx",
                    "content_hash": "hash",
    ...
    result = await DocumentAdapter(client).search(
        RetrievalRequest(question="operating plan", terms=["operating", "plan"]), development_context(AgentScope.ADMIN)
    )
    assert result.evidence
...
async def test_postgrest_or_filters_keep_parentheses():
            ...
            "documents": [
                {
                    ...
                    "title": "Operating Plan",
                    "source_type": "onedrive_filesystem",
                    "relative_path": "01_Governance/plan.docx",
                    "source_area": "01_governance",
                    "raw_text": "speaker plan",
                    "content_hash": "hash",
                    "filesystem_modified_at": "2026-08-01T00:00:00Z",
                }
            ]
    ...
    await DocumentAdapter(client).search(
        RetrievalRequest(question="speaker plan", terms=["speaker", "plan"]), development_context(AgentScope.ADMIN)
    )
    params = client.calls[0][2]
    or_value = next(value for key, value in params if key == "or")
    assert (
        or_value
        == "(title.ilike.*speaker*,relative_path.ilike.*speaker*,raw_text.ilike.*speaker*,source_area.ilike.*speaker*,title.ilike.*plan*,relative_path.ilike.*plan*,raw_text.ilike.*plan*,source_area.ilike.*plan*)"
    )
