...
async def retrieval_contract_health(client: SupabaseRestClient, *, probe: bool = True) -> dict[str, Any]:
    ...
    try:
        status_code, rows = await client.probe_rpc(RPC_NAME, {
    ...
    if status_code >= 400:
        category = "rpc_not_found" if status_code == 404 else "authentication_rejected" if status_code in {401, 403} else "rpc_probe_failed"
        return {**base, "status": "unavailable", "http_status": status_code, "error_category": category}
    try:
        validate_result_rows(rows)
    except ValueError as exc:
        return {**base, "status": "unavailable", "http_status": status_code, "error_category": "contract_schema_invalid", "contract_error": str(exc)}
    return {**base, "status": "ready", "http_status": status_code, "probe_result_count": len(rows),
