...
class DocumentAdapter:
    ...
    async def search(self, request: RetrievalRequest, auth: AuthContext) -> RetrievalResult:
        ...
        if scope is AgentScope.ADMIN:
            source_type = "onedrive_filesystem"
        elif can_read_source(scope, "documents_legacy"):
            source_type = "legacy"
        else:
            return RetrievalResult(source=self.source, queried=False, note="documents are not readable at this scope")
        params = [
            ("is_missing", "eq.false"),
            ("is_unavailable", "eq.false"),
            ("limit", str(min(max(request.max_results * 8, 24), 100))),
        ]
        if source_type == "onedrive_filesystem":
            params.append(("source_type", "eq.onedrive_filesystem"))
        else:
            params.append(("source_type", "not.eq.onedrive_filesystem"))
        ...
        try:
            rows = await self.client.select(
                "documents",
                "id,title,source_type,source_url,relative_path,source_area,raw_text,content_hash,filesystem_modified_at,current_version_id,metadata",
                params,
            )
        ...
        for row in rows:
            ...
            if row.get("current_version_id") and row.get("source_type") == "onedrive_filesystem":
                try:
                    versions = await self.client.select(
                        "document_versions",
                        "raw_text,content_hash,source_modified_at",
                        [("id", f"eq.{row['current_version_id']}"), ("limit", "1")],
                    )
            ...
            citation = Citation(
                ...
                title=text(row.get("title"), "EFDS document"),
                url=row.get("source_url"),
                path=path,
                source_updated_at=row.get("filesystem_modified_at"),
                content_hash=row.get("content_hash"),
                authority="canonical_document",
            )
            item = evidence(
                citation,
                body,
                source_article_relevance(
                    row.get("title"),
                    f"{row.get('relative_path', '')} {row.get('source_area', '')} {body}",
                    request.terms,
                    0.45,
                ),
                {
