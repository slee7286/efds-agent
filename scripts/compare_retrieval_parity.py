...
def _compare(canonical: dict[str, Any], actual: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "query": canonical.get("query"), "expected_count": len(expected), "actual_count": len(received),
        ...
        "rank_differences": [{"retrieval_unit_id": value, "canonical_rank": expected.index(value) + 1,
                              "agent_rank": received.index(value) + 1}
                             for value in expected if value in received and expected.index(value) != received.index(value)],
    }
...
async def run(args: argparse.Namespace) -> int:
    ...
    async with httpx.AsyncClient(timeout=args.timeout) as client:
        for case in canonical.get("cases", []):
            payload = {"query": case["query"], "scope": case.get("scope", args.scope),
                       "source_mode": args.source_mode, "conversation": []}
            try:
                response = await client.post(args.agent_url.rstrip("/") + "/v1/retrieval", headers=headers, json=payload)
                body = response.json()
                if response.status_code >= 400:
                    report["cases"].append({"query": case["query"], "status": "INTEGRATION_FAILURE", "http_status": response.status_code,
                ...
                report["cases"].append({"query": case.get("query"), "status": "INTEGRATION_FAILURE", "detail": type(exc).__name__})
    ...
    report["top10_set_match_rate"] = sum(item.get("top10_set_match", False) for item in report["cases"]) / len(report["cases"]) if report["cases"] else 0.0
    Path(args.output).parent.mkdir(parents=True, exist_ok=True)
    Path(args.output).write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"output": args.output, "exact_order_match_rate": report["exact_order_match_rate"],
                      "top10_set_match_rate": report["top10_set_match_rate"]}, indent=2))
